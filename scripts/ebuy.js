...
function homeform()
{
	...
	{
		costresult(ProductForm)
		alert("Your Total Price: $" + costresult(ProductForm));
	}
}		
...
function costresult(ProductForm)
{
	var product = ProductForm.selectproduct.value;
	var quantity = ProductForm.Quantity.value;
	if (product == 'Green Razer Keyboard - $100')
	{
		result = (quantity * 100);
		return result;
	}
	
	if (product == 'AMD Computer Desktop - $300')
	{
		result = (quantity * 300);
		return result;
	}
	
	if (product == 'Logitech Wireless Mouse - $30')
	{
		result = (quantity * 30);
		return result;
	}
	
	if (product == 'Sony Stereo Headphones - $20')
	{
		result = (quantity * 20);
		return result;
	}
